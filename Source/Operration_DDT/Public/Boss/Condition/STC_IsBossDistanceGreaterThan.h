...
#include "STC_IsBossDistanceGreaterThan.generated.h"

/**
 * @brief 보스가 범위 밖 거리에 있는지 확인하는 조건 클래스
 * 
 * StateTree에서 사용되는 조건 클래스로, Evaluator에서 설정된
 * CurrentRangeTag가 OutOfRange 태그와 일치하는지 확인합니다.
 */
UCLASS()
class OPERRATION_DDT_API USTC_IsBossDistanceGreaterThan : public UStateTreeConditionBlueprintBase
{
	GENERATED_BODY()
